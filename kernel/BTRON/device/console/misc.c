...
*/
/* $Header: /Users/tonpa/Downloads/b-free/b-free/kernel/BTRON/device/console/misc.c,v 1.1.1.1 2003/02/05 03:56:54 monaka Exp $ */
...
void move_curpos(W x, W y)
{
    ...
    if (curpos.x < 0)
	curpos.x = 0;
    else if (curpos.x >= MAX_COLUMN)
	curpos.x = MAX_COLUMN - 1;
    curpos.y += y;
    if (curpos.y < 0)
	curpos.y = 0;
    else if (curpos.y >= MAX_COLUMN)
	curpos.y = MAX_LINE - 1;
...
ER write_char(UB ch)
{
	...
	write_vram_xy(curpos.x, curpos.y, ch, attr);
	curpos.x++;
...
static void backspace()
{
    ...
    write_vram_xy(curpos.x, curpos.y, ' ', attr);
    goto_cursol(curpos.x, curpos.y);
}
...
#define CURSOR_POS(x,y)		(x + y * 80)
    ...
    switch (attr) {
    case REVERSE:
	attr_bits = VGA_TEXT_ATTR(VGA_TEXT_BLACK, VGA_TEXT_WHITE) << 8;
	break;
    case NORMAL:
	attr_bits = VGA_TEXT_ATTR(VGA_TEXT_WHITE, VGA_TEXT_BLACK) << 8;
	break;
    default:
	attr_bits = addr[CURSOR_POS(x, y)] & 0xff00;
	break;
    }
    ch = ch | attr_bits;
...
static void scroll_up(void)
{
	...
	write_vram_xy(i, MAX_LINE - 1, ' ', NORMAL);
    }
}
