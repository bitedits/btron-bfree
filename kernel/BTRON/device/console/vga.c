...
*/
/* $Header: /Users/tonpa/Downloads/b-free/b-free/kernel/BTRON/device/console/vga.c,v 1.1.1.1 2003/02/05 03:56:54 monaka Exp $ */
...
W graphic_command(ID caller, DDEV_CTL_REQ * packet)
{
    union gr_cmd_t *p;
    W errno;
    ...
    case GR_LINE:
	line_vga(p->line.x1, p->line.y1, p->line.x2, p->line.y2,
		 p->line.color, p->line.mode);
	return (E_OK);

    case GR_BOX:
	box_vga(p->box.x, p->box.y, p->box.width, p->box.height,
		p->box.color, p->box.mode);
	return (E_OK);

    case GR_FILLBOX:
	fillbox_vga(p->fillbox.x, p->fillbox.y, p->fillbox.width,
		    p->fillbox.height, p->fillbox.color, p->fillbox.mode);
	return (E_OK);

    case GR_CIRCLE:
	circle_vga(p->box.x, p->box.y, p->box.width, p->box.height,
		   p->box.color, p->box.mode);
	return (E_OK);
    ...
    case GR_SETBG:
	{
	    ...
	    errno =
		vget_reg(p->setbg.caller, p->setbg.bitmap, p->setbg.size,
			 copy_buffer);
	    if (errno) {
		return (errno);
	    }

	    setbg_vga(p->setbg.width, p->setbg.height, p->setbg.size,
		      copy_buffer, p->setbg.frcolor, p->setbg.bgcolor);
#ifdef USE_MALLOC
	    free(buf);
#endif
	}
    ...
    case GR_SETBITMAP:
	{
	    ...
	    errno =
		vget_reg(p->setbitmap.caller, p->setbitmap.bitmap,
			 p->setbitmap.size, copy_buffer);
	    if (errno) {
		return (errno);
	    }

	    stick_bitmap(p->setbitmap.x, p->setbitmap.y,
			 p->setbitmap.width, p->setbitmap.height,
			 copy_buffer, p->setbitmap.frcolor,
			 p->setbitmap.bgcolor);
#ifdef USE_MALLOC
	    free(buf);
#endif
    ...
    case GR_SPLBITMAP:
	{
	    ...
	    errno =
		vget_reg(p->splbitmap.caller, p->splbitmap.bitmap,
			 p->splbitmap.size, copy_buffer);
	    if (errno) {
		return (errno);
	    }

	    stipple_bitmap(p->splbitmap.x, p->splbitmap.y,
			   p->splbitmap.width, p->splbitmap.height,
			   copy_buffer, p->splbitmap.frcolor);
#ifdef USE_MALLOC
	    free(buf);
#endif
	}
    ...
    case GR_ELLIPSE:
	ellipse_vga(p->ellipse.x, p->ellipse.y, p->ellipse.width,
		    p->ellipse.height, p->ellipse.color, p->ellipse.mode);
	return (E_OK);

    case GR_FILLELLIPSE:
	fillellipse_vga(p->ellipse.x, p->ellipse.y, p->ellipse.width,
			p->ellipse.height, p->ellipse.color,
			p->ellipse.mode);
	return (E_OK);

    case GR_SCROLL:
	scroll_vga(p->scroll.direc, p->scroll.x, p->scroll.y,
		   p->scroll.width, p->scroll.height, p->scroll.step,
		   p->scroll.color);
	return (E_OK);
    default:
	break;
...
void save_register(struct vga_setup_info *info)
{
    int i;
...
void clear_vga(int color)
{
    ...
    for (i = 0; i < 80 * 480; i++) {
	*gvram++ = 0;
	/* ここで書き込んでいる値 0 は、VGA コントローラは参照しない。
...
void
stick_bitmap(int x, int y, int x_length, int y_length,
	     unsigned char *bitmap, int color, int bkcolor)
{
    int i, j, k;
    int w, xl;
    unsigned char *gvram;
	...
	for (xi = 0; xi < x_length; xi++) {
		...
		for (yi = 0; yi < y_length; yi++) {
			...
			if (bitmap[(xi + (yi * width)) / 8] & mask) {
			    point_vga(xi + x, yi + y, color,
				      VGA_OVERWRITE);
			} else {
			    point_vga(xi + x, yi + y, bkcolor,
				      VGA_OVERWRITE);
			}
		    }
		}
...
void
stipple_bitmap(int x, int y, int x_length, int y_length,
...
{
    unsigned char *gvram =
	(unsigned char *) (VGA_VRAM_ADDRESS + P2G(x / 8U, y));
...
void poly_line(int *table, int color, int mode)
{
	...
	line_vga(table[i], table[i + 1], table[i + 2], table[i + 3], color,
		 mode);
    }
}


void box_vga(W x, W y, W width, W height, W color, W restop)
{
...
void fillbox_vga(W x, W y, W width, W height, W color, W restop)
{
    int i, j, k, w;
    unsigned char *gvram;
    unsigned char mask;
...
void circle_vga(W x, W y, W rx, W ry, W color, W restop)
{
    int dx, dy;
    int xx, yy;
    int ddx, ddy;
...
void setbg_vga(W width, W height, W size, VP bitmap, W color, W bgcolor)
{
    W x, y;
...
void ellipse_vga(W x, W y, W width, W height, W color, W restop)
{
    int cx, cy, px, py;
...
void fillellipse_vga(W x, W y, W width, W height, W color, W restop)
{
    int cx, cy, px, py;
    int a, b, x1, y1, x2, y2, r, r1, r2, r3;
...
/* scroll_func */
void scroll_vga(W direc, W x, W y, W width, W height, W step, W color)
{
    W end, i, j;
    W xdiv8, k;
    W sr, ds, w;
