...
/* $Id: graphic.c,v 1.1.1.1 2003/02/05 03:56:57 monaka Exp $ */

static char rcsid[] =
    "$Header: /Users/tonpa/Downloads/b-free/b-free/kernel/BTRON/init/libCmd/graphic.c,v 1.1.1.1 2003/02/05 03:56:57 monaka Exp $";
...
void check_wconsole()
{
    ER error;
    ID wconsole;
    extern int find_port(B *, ID *);

    error = find_port(WCONSOLE_DRIVER, &wconsole);
    if ((error == E_PORT_OK) && (stdout->device != wconsole)) {
	stdout->device = wconsole;
	stderr->device = wconsole;
	console = wconsole;
    }
}
...
ER
set_bitmap(int x, int y, int width, int height, int size,
...
ER
spl_bitmap(int x, int y, int width, int height, int size,
...
ER
scroll_vga(int direc, int x, int y, int width, int height, int step,
	   int color)
{
...
ER wd_load(char *file)
{
    ...
    req.header.mbfid = recv;
    req.header.msgtyp = DEV_CTL;
    req.body.ctl_req.cmd = WD_LOAD;
    req.body.ctl_req.len = strlen(file);
    bcopy(file, req.body.ctl_req.param, req.body.ctl_req.len + 1);
