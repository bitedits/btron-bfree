...
/* $Id: posix_if.c,v 1.1.1.1 2003/02/05 03:56:56 monaka Exp $ */

static char rcsid[] =
    "$Header: /Users/tonpa/Downloads/b-free/b-free/kernel/BTRON/init/posix_if.c,v 1.1.1.1 2003/02/05 03:56:56 monaka Exp $";
...
static W posix_create_task(struct posix_process *procinfo,
			   struct posix_lowlib_info *lowlibinfo);
#endif
...
W posix_readdir(W fileid, B * buf, W length)
{
    ER error;
    struct posix_request req;
    struct posix_response res;
...
#if 0
W posix_stat(W fileid, struct stat * st)
{
    W result;
...
W posix_newprocess(B * program)
{
    ...
    procinfo.data_size =
	ROUNDUP(exec_info.a_data + exec_info.a_bss, PAGE_SIZE);
 ...
 */
static W
posix_create_task(struct posix_process *procinfo,
		  struct posix_lowlib_info *lowlibinfo)
{
    ER err;
    T_CTSK task_info;
...
W posix_fork(void *main_func, void *signal_func)
{
    ER error;
    struct posix_request req;
    struct posix_response res;
...
#if 0
    for (main_task = MIN_USERTASKID; main_task <= MAX_USERTASKID;
	 main_task++) {
    ...
    task_info.stksz = 1024 * 12;
#if 0
    for (signal_task = MIN_USERTASKID; signal_task <= MAX_USERTASKID;
	 signal_task++) {
