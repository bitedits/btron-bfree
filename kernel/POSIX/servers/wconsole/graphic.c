...
*/
/* $Header: /Users/tonpa/Downloads/b-free/b-free/kernel/POSIX/servers/wconsole/graphic.c,v 1.1.1.1 2003/02/05 03:57:20 monaka Exp $ */
static char rcsid[] = "$Header: /Users/tonpa/Downloads/b-free/b-free/kernel/POSIX/servers/wconsole/graphic.c,v 1.1.1.1 2003/02/05 03:57:20 monaka Exp $";
...
void kfopen()
{
  ...
  if (has_kfont) return;
  if ((k_fd = open(KFONT, 0, 0)) < 0) {
    dbg_printf("WCONSOLE: cant open file %s\n", KFONT);
    return;
  }
#ifdef notdef
  ...
  dbg_printf("font size = %d\n", st.st_size);
  kf_data = (char *) malloc(st.st_size);
  read(k_fd, kf_data, st.st_size);
  fheader = *((struct font_header *) kf_data);
  close(k_fd);
#endif
  has_kfont = 1;
}
...
void draw_kch(int x, int y, int code, int color, int bgcolor)
{
  int oft;
#ifdef notdef
  char buf[255];
  ...
  if (code < fheader.top_code || code > fheader.end_code) {
    code = 0x2121;
  }
  oft = cal_offset(code);
  ...
  oft = cal_offset(code);
  buf = &kf_data[1024 + oft*fheader.fnt_size];
#endif
  set_bitmap(x, y, fheader.fnt_width, fheader.fnt_height, fheader.fnt_size,
	     buf, color, bgcolor);
}

void init_w_des()
{
  eval("var x var y var w var h"); EC;
  eval(": root 8 setcolor 0 0 640 480 rectf ;"); EC;
  eval(": shadow 15 setcolor 4 ndup 4 ndup moveto over 0 rlineto"); EC;
  eval("4 ndup 4 ndup moveto 0 over rlineto 8 setcolor"); EC;
  eval("over 0 rlineto 0 over minus rlineto 4 ndrop ;"); EC;
  eval(": window h $ w $ y $ x $ 7 setcolor"); EC;
  eval("x @ 10 - y @ 22 - w @ 20 + h @ 32 + rectf"); EC;
  eval("x @ 10 - y @ 22 - w @ 20 + h @ 32 + shadow"); EC;
  eval("x @ 4 - y @ 4 - w @ 8 + h @ 8 + shadow"); EC;
  eval("15 setcolor x @ 1- y @ 1- w @ 1+ h @ 1+ rect "); EC;
  eval("x @ y @ w @ h @ rectf 7 setcolor"); EC;
  eval("x @ 50 + y @ 28 - 154 19 rectf x @ 50 + y @ 28 - 154 23 shadow"); EC;
  eval("0 setcolor x @ 57 + y @ 25 - 140 18 rectf ;"); EC;
  eval(": title h $ w $ y $ x $ 15 setcolor 1 setfont"); EC;
  eval("x @ 60 + y @ 23 - SP nrotr SP nrotr drawstr ;"); EC;
  dbg_printf("defined default window\n");
}
...
void draw_window()
{
  ...
  for (i = 0; i < max_win; ++i) {
    if (w[i].map) {
      ...
      box_vga(w[i].x-1, w[i].y-1, w[i].width+1, w[i].height+1, 15, 0);
      fillbox_vga(w[i].x, w[i].y, w[i].width, w[i].height, 15, 0);
      fillbox_vga(w[i].x+50, w[i].y-w[i].font_h*2,
		  w[i].font_w * 22, w[i].font_h+5, 7, 0);
      draw_shadow(w[i].x+50, w[i].y-w[i].font_h*2,
		  w[i].font_w * 22, w[i].font_h*2-5);
      fillbox_vga(w[i].x+50+w[i].font_w, w[i].y-w[i].font_h*2+3,
		  w[i].font_w * 20, w[i].font_h+4, 0, 0);
...
void wd_load(char *file)
{
  f_forgetall(); EC;
  push_cstring(file); EC;
  eval("load"); EC;
}
