...
/* 
 * $Header: /Users/tonpa/Downloads/b-free/b-free/kernel/POSIX/mkfs/sfsck.c,v 1.1.1.1 2003/02/05 03:57:20 monaka Exp $
 *
 * $Log: sfsck.c,v $
 * Revision 1.1.1.1  2003/02/05 03:57:20  monaka
 * From eota-0.3.4.tgz
 *
...
/* copy from statfs.c */
W read_inode(int fd, struct sfs_superblock *sb, int ino,
	     struct sfs_inode *ip);
int get_inode_offset(struct sfs_superblock *sb, int ino);
void print_superblock(struct sfs_superblock *sb);
W read_dir(int fd, struct sfs_superblock *sb, struct sfs_inode *ip,
	   int nentry, struct sfs_dir *dirp);
int read_file(int fd, struct sfs_superblock *sb, struct sfs_inode *ip,
	      int start, int size, B * buf);
int read_block(int fd, int blockno, int blocksize, B * buf);
int get_block_num(int fd, struct sfs_superblock *sb, struct sfs_inode *ip,
		  int blockno);
int get_indirect_block_num(int fd, struct sfs_superblock *sb,
			   struct sfs_inode *ip, int blockno);
int get_dindirect_block_num(int fd, struct sfs_superblock *sb,
			    struct sfs_inode *ip, int blockno);
...
int main(int argc, char *argv[])
{
	...
	if (bitmap_used[i] > 1) {
	    fprintf(stderr, "WARNING: File System Inconsistent!!\n");
	    fprintf(stderr,
		    "file system contains doubly allocated block\n");
	    free(inode);
	    free(bitmap_disk);
	    free(bitmap_used);
	    return (-1);
	    ...
	    fprintf(stderr, "WARNING: File System Inconsistent!!\n");
	    fprintf(stderr, "file system contains unfreed block %d\n", i);
	    free(inode);
	    free(bitmap_disk);
	    free(bitmap_used);
	    return (-1);
	    ...
	    fprintf(stderr, "WARNING: File System Inconsistent!!\n");
	    fprintf(stderr, "file system looses using block %d\n", i);
	    free(inode);
	    free(bitmap_disk);
	    free(bitmap_used);
	    return (-1);
...
W
read_inode(int fd, struct sfs_superblock *sb, int ino,
	   struct sfs_inode *ip)
{
    int offset, errno;
...
read_dir(int fd,
	 struct sfs_superblock * sb,
	 struct sfs_inode * ip, int nentry, struct sfs_dir * dirp)
{
    int size;
...
read_file(int fd,
	  struct sfs_superblock *sb,
	  struct sfs_inode *ip, int start, int size, B * buf)
{
    B *blockbuf;
    int copysize;
    int offset;
...
int
get_indirect_block_num(int fd, struct sfs_superblock *sb,
		       struct sfs_inode *ip, int blockno)
{
    int inblock;
    int inblock_offset;
...
int
get_dindirect_block_num(int fd, struct sfs_superblock *sb,
			struct sfs_inode *ip, int blockno)
{
    int dinblock;
    int dinblock_offset;
